...
* **FULLY WIRED (ready for #57 to run):**
  ...
  `test-secure-boot-bad-sig-refused`, `test-secure-boot-staged` (#2 — the
  staged-boot/priority-volume apply, FEATURE #2), plus the already-landed
...
* **STUBBED (precise TODOs above):** wrong-key (#4c), domain-transplant (#4d),
  ...
  priority-ok (#5b), bad-priority (#5c), driver-image (#1).
