#!/usr/bin/env bash
# Secure-boot NEGATIVE scenarios that differ from sb-bad-sig-refused.sh (#4b)
# only in how the staged disk is prepared, selected by NMBL_SB_TAMPER:
#
#   wrong-key          (#4c) every generation sidecar is replaced by a VALID
#                      signature from a foreign ML-DSA key that is NOT baked
#                      into NMBL. Any-of verification over the baked keys must
#                      reject it.
#   domain-transplant  (#4d) every kernel sidecar is replaced by a VALID
#                      signature from the BAKED key over the same bytes, but
#                      under the driver-image domain. Per-role domain
#                      separation must reject it as a gen-kernel signature.
#   sentinel           (#5a) the generation is left intact but an empty rescue
#                      sentinel /boot/nmbl/rescue is dropped on the ESP. NMBL
#                      must skip the measured boot entirely, seal the TPM, and
#                      go to rescue; the valid generation must never boot.
#   bad-priority       (#5c) boots the staged-boot twin, whose inside-LUKS
#                      priority volume carries a signed priority file. The
#                      file is overwritten (inside LUKS, opened with the
#                      install passphrase) so its signature no longer matches.
#                      The post-unlock gate must refuse: cap the TPM, close the
#                      cryptroot mapper, write the sentinel, reboot. The
#                      generation never boots and no shell is offered.
#
# The two signature modes share the refuse assertions below with #4b. The
# sentinel mode asserts the sentinel rescue instead of a signature refuse.
# Original #4b notes follow.
#
#
...
CONFIG_NAME="${NMBL_SB_CONFIG_NAME:-test-secure-boot-enroll}"
...
TAMPER_MODE="${NMBL_SB_TAMPER:?set NMBL_SB_TAMPER to wrong-key, domain-transplant or sentinel}"
case "$TAMPER_MODE" in
  wrong-key|domain-transplant) ;;
  bad-priority)
    REFUSE_RE='SECURE BOOT: REFUSED|refused by secure-boot policy'
    # The relock of cryptroot (policy/relock.rs) runs while NMBL holds the
    # console, so its log line never reaches serial and a refused boot never
    # reaches a journal. The relock ORDER is covered by the policy unit tests
    # (cap -> close -> sentinel -> relock); this scenario asserts the observable
    # outcome: refused, no shell, the generation never boots.
    ;;
  sentinel)
    # The sentinel forces rescue before any generation is measured or kexec'd.
    # PASS needs the forced-rescue marker; the booted system must never appear.
    REFUSE_RE='rescue sentinel present|entering rescue without operator input'
    # Entering rescue must seal first: the lock PCR is capped before the
    # rescue shell (policy/guard.rs logs it).
    REQUIRED_AFTER_REFUSE_RE='seal: lock PCR capped'
    # A sentinel-forced rescue legitimately enters the (sealed) rescue shell, so
    # the refuse-terminus "no shell" rule does not apply; what must never happen
    # is the valid generation booting, which BOOTED_RE catches unconditionally.
    EMERGENCY_SHELL_RE='NMBL_NEVER_MATCHES_A_SENTINEL_RESCUE'
    ;;
  *) echo "FAIL: unknown NMBL_SB_TAMPER '$TAMPER_MODE'" >&2; exit 1 ;;
esac
...
# PREPARE the staged disk for the selected mode. The disko-luks layout is:
# sda1 = EF02 boot, sda2 = vfat ESP (/boot), sda3 = LUKS. Sidecars and the
# sentinel live on the ESP, outside LUKS, so no disk key is needed.
SIGNER="${NMBL_SB_SIGNER:?NMBL_SB_SIGNER must name the nmbl-sign binary}"
# Enumerate the generation sidecars (paths relative to /nmbl/sigs).
SIG_LIST="${RUN_DIR}/siglist.txt"
if ! guestfish --ro -a "${RUN_DIR}/${CONFIG_NAME}.qcow2" >"${SIG_LIST}" <<'GF'
run
mount-ro /dev/sda2 /
find /nmbl/sigs
umount /
...
mapfile -t SIDECARS < <(grep -E '/(kernel|initrd)\.sig$' "${SIG_LIST}" | sed -e 's#^/*#/nmbl/sigs/#' || true)
if [ "${#SIDECARS[@]}" -eq 0 ]; then
  echo "FAIL: no generation sidecars under /nmbl/sigs — the staged disk is unsigned" >&2
  exit 1
fi

# The verifier checks the generation's store kernel and initrd, which live in
# the LUKS-protected root. Re-signing needs the SAME bytes, so the flake exports
# them from the scenario config as NMBL_SB_GEN_KERNEL / NMBL_SB_GEN_INITRD.
GEN_KERNEL="${NMBL_SB_GEN_KERNEL:-}"
GEN_INITRD="${NMBL_SB_GEN_INITRD:-}"

GF_SCRIPT="${RUN_DIR}/prep.gf"
{ echo "run"; echo "mount /dev/sda2 /"; } >"$GF_SCRIPT"
case "$TAMPER_MODE" in
  wrong-key|domain-transplant)
    [ -f "$GEN_KERNEL" ] && [ -f "$GEN_INITRD" ] || { echo "FAIL: NMBL_SB_GEN_KERNEL/INITRD unset" >&2; exit 1; }
    ;;
esac
case "$TAMPER_MODE" in
  wrong-key)
    # A fresh foreign ML-DSA-87 key that exists only on this pipe/tmp dir.
    "$SIGNER" keygen --alg ml-dsa-87 --stdio > "${RUN_DIR}/foreign.key" 3> "${RUN_DIR}/foreign.pub"
    "$SIGNER" sign --key "${RUN_DIR}/foreign.key" --domain gen-kernel \
      --out "${RUN_DIR}/kernel.sig" "$GEN_KERNEL" >/dev/null
    "$SIGNER" sign --key "${RUN_DIR}/foreign.key" --domain gen-initrd \
      --out "${RUN_DIR}/initrd.sig" "$GEN_INITRD" >/dev/null
    # The foreign signatures really are valid ones, just not for a baked key.
    "$SIGNER" verify --key "${RUN_DIR}/foreign.pub" --domain gen-kernel \
      --sig "${RUN_DIR}/kernel.sig" "$GEN_KERNEL" >/dev/null
    for sc in "${SIDECARS[@]}"; do
      case "$sc" in
        */kernel.sig) echo "upload ${RUN_DIR}/kernel.sig ${sc}" ;;
        */initrd.sig) echo "upload ${RUN_DIR}/initrd.sig ${sc}" ;;
      esac
    done >>"$GF_SCRIPT"
    ;;
  domain-transplant)
    # The BAKED key, the SAME kernel bytes, but the driver-image domain.
    BAKED_KEY="${NMBL_SB_BAKED_KEY:?NMBL_SB_BAKED_KEY must name the baked test private key}"
    "$SIGNER" sign --key "$BAKED_KEY" --domain driver-image \
      --out "${RUN_DIR}/kernel.sig" "$GEN_KERNEL" >/dev/null
    for sc in "${SIDECARS[@]}"; do
      case "$sc" in */kernel.sig) echo "upload ${RUN_DIR}/kernel.sig ${sc}" ;; esac
    done >>"$GF_SCRIPT"
    ;;
  bad-priority)
    # The priority file lives INSIDE LUKS on cryptroot; open it with the
    # fixed install passphrase and overwrite the file's head so the detached
    # priority-file signature no longer verifies (the file stays present).
    PRIORITY_FILE="${NMBL_SB_PRIORITY_FILE:?}"
    printf 'NMBL_BAD_PRIORITY_TAMPER' >"${RUN_DIR}/priority-tamper.bin"
    # The LUKS2 keyslot is argon2id with ~560 MiB memory cost: give the
    # libguestfs appliance 2 GiB, and open it with cryptsetup inside the
    # appliance (the guestfish --key path does not derive this keyslot).
    LIBGUESTFS_MEMSIZE=2048 guestfish --rw -a "${RUN_DIR}/${CONFIG_NAME}.qcow2" <<GF || { echo "FAIL: could not tamper the priority file inside LUKS" >&2; exit 1; }
run
debug sh "printf '%s' '${LUKS_PASSPHRASE}' | cryptsetup open --key-file=- /dev/sda3 nmbltamper"
mount /dev/mapper/nmbltamper /
upload-offset ${RUN_DIR}/priority-tamper.bin ${PRIORITY_FILE} 0
umount /
debug sh "cryptsetup close nmbltamper"
GF
    ;;
  sentinel)
    : >"${RUN_DIR}/sentinel"
    echo "mkdir-p /nmbl" >>"$GF_SCRIPT"
    echo "upload ${RUN_DIR}/sentinel /nmbl/rescue" >>"$GF_SCRIPT"
    ;;
esac
echo "umount /" >>"$GF_SCRIPT"
if ! guestfish --rw -a "${RUN_DIR}/${CONFIG_NAME}.qcow2" -f "$GF_SCRIPT"; then
  echo "FAIL: guestfish could not prepare the ${TAMPER_MODE} disk" >&2
  cat "$GF_SCRIPT" >&2 || true
  exit 1
fi
rm -f "${RUN_DIR}/foreign.key"
echo "=== ${TAMPER_MODE} disk prepared ===" >&2
...
check_no_unrefused_boot() {
  if [ "$TAMPER_MODE" = sentinel ] && [ -n "$(first_match_line "$BOOTED_RE")" ]; then
    echo "FAIL: the generation booted although the rescue sentinel was present" >&2
    return 1
  fi
...
if [ -n "${REQUIRED_AFTER_REFUSE_RE:-}" ] && ! seen_in_history "$REQUIRED_AFTER_REFUSE_RE"; then
  echo "FAIL: the refuse did not relock cryptroot (expected '$REQUIRED_AFTER_REFUSE_RE')" >&2
  exit 1
fi
