...
in
{
  ...
  config = {
    ...
    # 7}. Warn (not fail) when measuring with `pcrIndex = 7`, which
    # collapses the two into one PCR and stops separately binding NMBL's
    # boot events; this usually signals a mis-pinned `pcrIndex`.
    warnings = lib.optional (tpm.measure && pcrSetDegenerate) ''
