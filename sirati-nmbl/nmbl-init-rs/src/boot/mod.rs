...
pub fn kexec_into<S: SysOps>(
    ...
    // to the RebootIntoRescue terminus (R-1) — no image is loaded. The LOAD
    // (and the pre-handoff sync(2)+settle) is routed through the `KexecOps`
    // seam so a dry-run impl can no-op it without dropping verify/measure.
