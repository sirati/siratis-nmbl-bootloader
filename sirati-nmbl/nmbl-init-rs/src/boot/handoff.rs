...
pub(crate) fn verify_measure_then_load<S: SysOps>(
    ops: &mut S,
    config: &Config,
    generation: &Generation,
    cmdline_override: Option<&str>,
    ...
    // verified fd (non-secure-boot / audit), the loader opens by path. The
    // load (and pre-handoff sync(2)+settle) is routed through `ops` so a
    // dry-run impl can no-op it.
    super::handoff_load::load_handoff(ops, generation, verified, fragment.as_slice(), &cmdline)?;
    Ok(cmdline)
}
