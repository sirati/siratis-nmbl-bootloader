...
/// (c) LOAD the verified image through the [`KexecOps`](crate::sys::ops::KexecOps)
/// seam so a dry-run impl can no-op the load. On the secure-boot path the PINNED
/// kernel + initrd source fds from `verified` are handed straight to
/// `kexec_file_load(2)` (FIX-02 / MED-1 / LOW-A) so the loaded image is the exact
/// one verified+measured; otherwise the loader opens by path. The initrd is
/// always combined with the NMBL cpio `fragment` in a memfd (the typed
/// passphrases + log transcript never touch disk). The genuine impl also runs the
/// pre-handoff `sync(2)` + settle.
#[cfg_attr(
    not(feature = "secure-boot"),
    expect(
        unused_variables,
...
pub(super) fn load_handoff<S: SysOps>(
    ops: &mut S,
    generation: &Generation,
    verified: Option<VerifiedGeneration>,
    fragment: &[u8],
    ...
    let target = KexecTarget::MultiFile {
    ...
    // spliced with the cpio fragment are read from the verified initrd fd, so
    // they are byte-identical to the verified + measured ones. When there is no
    // verified generation (non-secure-boot / audit / signing-disabled) the
    // loader opens both by path.
    #[cfg(feature = "secure-boot")]
    if let Some(vg) = verified {
        return ops.kexec_load(
            target,
            Some((vg.kernel_fd.as_fd(), vg.initrd_fd.as_fd())),
            fragment,
            cmdline,
            0,
        );
    }
    ops.kexec_load(target, None, fragment, cmdline, 0)
}
