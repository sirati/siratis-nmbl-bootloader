...
pub trait Console {
    ...
    fn poll_key(&mut self, timeout: Duration) -> Result<Option<KeyEvent>> {
        match self.poll_event(timeout)? {
            ...
            // Scroll events only matter to scrollback-aware consumers
            // (the pretty shell) that call `poll_event` directly. The
            // caller asked for a key, so report no key this slice.
