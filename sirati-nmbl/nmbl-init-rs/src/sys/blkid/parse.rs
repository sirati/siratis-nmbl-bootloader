...
pub(super) fn blkid_for(dev: &Path) -> Result<HashMap<String, String>> {
    // `-p` selects LOW-LEVEL probing (bypass the blkid cache). NMBL has
    // no udev, so the cache-backed default mode returns only superblock
    ...
    // low-level mode reads the GPT directly, but emits partition info
    // under `PART_ENTRY_*` keys; `parse_blkid_export` aliases those back
    // to the cache-mode `PARTLABEL`/`PARTUUID` names the symlink layer
    // expects.
    let argv = vec![
        "-p".to_string(),
        "-o".to_string(),
