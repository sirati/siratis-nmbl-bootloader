...
///
/// Requires a [`crate::policy::Sealed`] by value so the type system
/// enforces the seal-before-spawn invariant at the real fork/execve
/// waist (re-audit C-1 / FIX-03), not just the `nmbl-init-must-seal`
/// lint.
///
...
pub fn spawn_shell_on_tty(
    ...
    // Consumed by value to gate this call site; carries no runtime state.
    let _sealed = sealed;
...
mod tests {
    ...
    fn open_tty_fd_on_occupied_tty_errors_without_hanging() {
        ...
        // The mount routes through the sync `FsOps` seam (no poller needed).
        if super::super::ensure_devpts_mounted(&mut crate::sys::ops::RealSys::sync_only()).is_err()
        {
            return;
        }
