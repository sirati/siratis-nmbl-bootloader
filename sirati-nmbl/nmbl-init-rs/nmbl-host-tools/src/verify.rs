...
pub fn run(input: &Path, public_key: &Path, domain: &'static [u8], sig: &Path) -> Result<()> {
    let sidecar_bytes = fs::read(sig)
        .map_err(|e| SignError::io(format!("read sidecar {}", sig.display()), e))?;
    ...
    let file = fs::File::open(input)
        .map_err(|e| SignError::io(format!("open {}", input.display()), e))?;
    let mut reader = std::io::BufReader::new(file);
    let mut hasher = Sha512::new();
    std::io::copy(&mut reader, &mut hasher)
        .map_err(|e| SignError::io(format!("hash {}", input.display()), e))?;
    let digest: [u8; 64] = hasher.finalize().into();
    verify_digest(&digest, domain, &sidecar, &[key], VerifyPolicy::Enforce)
        .map_err(|e| SignError::Crypto {
            context: format!("verify {}", input.display()),
            reason: e.to_string(),
        })?;
