...
|-------------------------------------|------------------:|------------------:|-------------------:|
| embedded (back-compat default)      |        37,570,131 |         1,351,920 |                n/a |
| external-config                     |        37,570,313 |         1,351,920 |                n/a |
| external-rescue                     |        37,624,383 |         1,351,920 |            700,416 |
| external-rescue + network           |        38,253,009 |         1,810,704 |            700,416 |

Notes:

- **embedded** is `test-gpt-uefi-grub` — the back-compat baseline
  with `configLocation = "embedded"` and `rescue.mode = "none"` (the
  defaults). Everything ships inside the initramfs.
- **external-config** (`test-external-config`) is byte-for-byte the
  same as embedded except the runtime config TOML on the initramfs
  shrinks to the bootstrap descriptor while the full config.toml is
  staged onto the boot partition by `lib/install-bootloader.nix`.
  The +182-byte delta is just the bootstrap.toml header overhead vs
  the full config.toml — both files live under `/etc/nmbl/` and
  compress nearly identically with gzip -9.
- **external-rescue** (`test-external-rescue`) sets
  ...
  The initramfs grows by ~54 KiB versus embedded because two extra
  kernel modules (`loop` + `squashfs`) get pulled into the closure
  to support loop-mounting the rescue blob at runtime. The squashfs
  itself adds ~684 KiB of staged content to the boot partition.
- **external-rescue + network** (`test-external-rescue-network`)
  flips `rescue.network = true` on top of external-rescue. The
  initramfs grows by ~614 KiB versus external-rescue: ~459 KiB
...
**Not met.** External-config saves zero bytes (the bootstrap TOML
swap is negligible), and external-rescue actually grows the
initramfs by ~54 KiB because the loop+squashfs modules need to be
present so `nmbl-init` can `LOOP_CTL_GET_FREE` + mount the rescue
blob. The plan's expectation assumed busybox and storage activation
tooling would move out of the initramfs along with the rescue blob,
but the current implementation keeps busybox unconditionally in
`baseContents` (it's still needed as the emergency shell before the
rescue squashfs is loop-mounted) and only conditionally carries
LVM/LUKS/mdadm/zfs binaries when `boot.nmbl.activation.*` requests
them — none of which the test configs trigger. The external rescue
path's value is therefore in being able to ship a much larger
rescue toolbox (full strace, GDB, etc.) on the boot partition without
inflating the initramfs, rather than in shrinking the initramfs
itself. Operators who want the smaller initramfs need to also gate
the embedded busybox on `rescue.mode != "external"`, which is a
separate refactor.
